import type { AdventureTask, BuildTemplate, CalculatedSheet, CanonicalEquipment, CanonicalSystem, Champion, ChampionLoadout, Hero, LineupSystem, PartyUnit, SimulationProgress, SimulationResult } from "../types/domain";
...
const fromChampionEquipment = (equipment: CanonicalEquipment | null | undefined): ChampionLoadout["familiarEquipment"] => equipment ? ({
  ...(equipment.itemId ? { itemId: equipment.itemId } : {}), ...(equipment.name == null ? {} : { name: equipment.name }),
  quality: reverseQualities[equipment.quality],
  ...(equipment.element == null ? {} : { element: equipment.element }), ...(equipment.spirit == null ? {} : { spirit: equipment.spirit }),
...
export function toCanonicalSystem(system: LineupSystem): CanonicalSystem {
  ...
  return {
    ...
    adventureTasks: system.taskGroups.flatMap((group) => group.tasks.map((task) => ({
      ...
      maxMembers: task.maxMembers, barrier: task.barrier, config: task.config, ...(task.result ? { result: task.result } : {}),
      modifiers: [], simulation: task.result ? {
        simulatorVersion: task.result.simulatorVersion,
        gameDataVersion: task.result.gameDataVersion,
...
export function fromCanonicalSystem(system: CanonicalSystem): LineupSystem {
  const toTask = (task: CanonicalSystem["adventureTasks"][number]) => ({
    ...
    config: task.config, ...(task.result ? { result: task.result } : {}),
  });
  ...
  const taskGroups = [...system.groups].sort((a, b) => a.sortOrder - b.sortOrder).map((group) => ({
    ...
    tasks: system.adventureTasks.filter((task) => task.groupId === group.id).map(toTask),
  }));
  const ungrouped = system.adventureTasks.filter((task) => !task.groupId || !groupIds.has(task.groupId));
  if (ungrouped.length) taskGroups.push({ id: crypto.randomUUID(), name: "未分组任务", tasks: ungrouped.map(toTask) });
  return {
    ...
    heroes: system.heroes.map((hero) => ({
      ...
      ...(hero.spritePath ? { spritePath: hero.spritePath } : {}), element: (hero.element || "光") as LineupSystem["heroes"][number]["element"],
      ...
      equipment: hero.equipment.map((equipment) => ({ ...(equipment.itemId ? { itemId: equipment.itemId } : {}), ...(equipment.name == null ? {} : { name: equipment.name }),
        slot: reverseSlots[equipment.slot], quality: reverseQualities[equipment.quality],
        ...(equipment.element == null ? {} : { element: equipment.element as LineupSystem["heroes"][number]["element"] }), ...(equipment.spirit == null ? {} : { spirit: equipment.spirit }),
        shiny: equipment.shiny, transcendence: equipment.transcendence || (equipment.transcended ? 1 : 0) })),
    })),
...
function loadedCatalog(): Catalog {
  if (!activeCatalog) throw new Error("完整本地目录尚未加载");
  return activeCatalog;
}

export function setBridgeCatalogForTests(catalog: Catalog): void {
  if (import.meta.env.MODE !== "test") throw new Error("测试目录注入仅允许在 test 模式使用");
...
export const desktopBridge = {
  ...
  async loadCatalog(): Promise<Catalog> {
    activeCatalog = await loadBrowserCatalog();
    getCatalogIndex(activeCatalog);
    return activeCatalog;
  },
  ...
  async calculateHero(hero: Hero): Promise<CalculatedSheet> {
    return calculateHeroSheet(loadedCatalog(), hero);
  },
  ...
  async calculateChampion(champion: Champion, loadout: ChampionLoadout, titanTower = false, catalog?: Catalog): Promise<CalculatedSheet> {
    return calculateChampionPreview(catalog ?? loadedCatalog(), champion, loadout, titanTower);
  },
  ...
  async simulate(
    ...
    const catalogIndex = getCatalogIndex(catalog);
    const quest = task.questId ? catalogIndex.quests.get(task.questId) : undefined;
    const result = await simulationQueue.enqueue(task.id, {
      task,
      ...
      ...(quest ? { quest } : {}),
      modifiers: catalog.simulationModifiers ?? {},
    }, onProgress, signal);
  ...
  async assetUrl(relativePath: string): Promise<string> {
    ...
    return `${import.meta.env.BASE_URL}content/${safe}`;
  },
};
